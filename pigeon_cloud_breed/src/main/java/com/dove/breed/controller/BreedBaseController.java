...
public class BreedBaseController {
    ...
    @PostMapping("/save")
    public Result save(@RequestBody BreedBase breedBase){
    ...
    @PostMapping("/get")
    public Result list(@RequestBody BreedBase breedBase){
        List<BreedBase> breedBaseList = breedBaseService.list(new QueryWrapper<>(breedBase));
        return breedBaseList.size() > 0?Result.success("查询成功").data(breedBaseList) : Result.error("查询失败");
    }
    ...
    public Result get(@PathVariable("id") String id){
        ...
        return breedBase == null? Result.success("查询成功").data(breedBase) : Result.error("查询失败");
    }

    @ApiOperation(value = "根据id修改")
    @PostMapping("/update/{id}")
    public Result update(@PathVariable("id") Long id, @RequestBody BreedBase breedBase){
        breedBase.setId(id);
