...
public class ShipmentEntryBaseController {
    ...
    @PostMapping("/save")
    public Result save(@RequestBody ShipmentEntryBase shipmentEntryBase){
    ...
    @PostMapping("/get")
    public Result list(@RequestBody ShipmentEntryBase shipmentEntryBase){
        List<ShipmentEntryBase> shipmentEntryBaseList = shipmentEntryBaseService.list(new QueryWrapper<>(shipmentEntryBase));
        return shipmentEntryBaseList.size() > 0?Result.success("查询成功").data(shipmentEntryBaseList) : Result.error("查询失败");
    }
    ...
    public Result get(@PathVariable("id") String id){
        ...
        return shipmentEntryBase == null? Result.success("查询成功").data(shipmentEntryBase) : Result.error("查询失败");
    }

    @ApiOperation(value = "根据id修改")
    @PostMapping("/update/{id}")
    public Result update(@PathVariable("id") Long id, @RequestBody ShipmentEntryBase shipmentEntryBase){
