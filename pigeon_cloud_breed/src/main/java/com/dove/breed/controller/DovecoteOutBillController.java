...
public class DovecoteOutBillController {
    ...
    @PostMapping("/save")
    public Result save(@RequestBody DovecoteOutBill dovecoteOutBill){
    ...
    @PostMapping("/get")
    public Result list(@RequestBody DovecoteOutBill dovecoteOutBill){
        List<DovecoteOutBill> dovecoteOutBillList = dovecoteOutBillService.list(new QueryWrapper<>(dovecoteOutBill));
        return dovecoteOutBillList.size() > 0?Result.success("查询成功").data(dovecoteOutBillList) : Result.error("查询失败");
    }
    ...
    public Result get(@PathVariable("id") String id){
        ...
        return dovecoteOutBill == null? Result.success("查询成功").data(dovecoteOutBill) : Result.error("查询失败");
    }

    @ApiOperation(value = "根据id修改")
    @PostMapping("/update/{id}")
    public Result update(@PathVariable("id") Long id, @RequestBody DovecoteOutBill dovecoteOutBill){
