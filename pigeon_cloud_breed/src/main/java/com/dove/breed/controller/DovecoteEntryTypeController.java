...
public class DovecoteEntryTypeController {
    ...
    @PostMapping("/save")
    public Result save(@RequestBody DovecoteEntryType dovecoteEntryType){
    ...
    @PostMapping("/get")
    public Result list(@RequestBody DovecoteEntryType dovecoteEntryType){
        List<DovecoteEntryType> dovecoteEntryTypeList = dovecoteEntryTypeService.list(new QueryWrapper<>(dovecoteEntryType));
        return dovecoteEntryTypeList.size() > 0?Result.success("查询成功").data(dovecoteEntryTypeList) : Result.error("查询失败");
    }
    ...
    public Result get(@PathVariable("id") String id){
        ...
        return dovecoteEntryType == null? Result.success("查询成功").data(dovecoteEntryType) : Result.error("查询失败");
    }

    @ApiOperation(value = "根据id修改")
    @PostMapping("/update/{id}")
    public Result update(@PathVariable("id") Long id, @RequestBody DovecoteEntryType dovecoteEntryType){
